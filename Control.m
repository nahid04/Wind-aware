...
syms gset next_r p0 p1 p2 p3 p4 inverse_diagonal Vmax maximum_value psvx psvy psvz
% In general -------------------------------------------------------
...
%gset= [gx; gy; gz];
   ...
   syms v_dot_com c2 error_v_dot error_v v_dot_com_x v_dot_com_y v_dot_com_z kx ky kz error_v_x error_v_y error_v_z roll_com pitch_com
% control law--------------------------------------------
c2 =1;
if c2 > 0 
  v_dot_com = (ft/m) * Rb * e3  + psv + g - c2 * [v_com - v]
else
    v_dot_com = 0
...
error_v = v - v_com;
% Exponentially stable error dynamics------------------------

error_v_dot = -c2*error_v;
v_dot_com = transpose ([v_dot_com_x v_dot_com_y v_dot_com_z]);

% definition of k's

kx =  -psvx - c2 * error_v_x + v_dot_com_x
ky = -psvy - c2 * error_v_y + v_dot_com_y
kz = -psvz - c2 * error_v_z + v_dot_com_z

%drones altitude controlling thrust, fta

fta = m * (kz/(cos(roll)*cos(pitch))) 

roll_com = asin(((kx * sin(yaw))- (ky * cos (yaw))) / fta)

pitch_com = asin(((kx * cos(yaw))- (ky * sin (yaw))) / (fta * cos(roll)))
...
syms ac ftc tau1 tau2 tau3 E Ecom H error_dot_E inv_H ce error_w  w_com b1i fi 

E = transpose ([roll pitch yaw])
Ecom = transpose ([roll_com pitch_com yaw_com])

error_E = E - Ecom
error_dot_E = H * w

inv_H = [1 0 -sin(pitch); 0 cos(roll) sin(roll)*cos(pitch); 0 -sin(roll) cos(roll)*cos(pitch)]
...
cw =1;
error_w = w - w_com;
w_com = inv_H * [-ce*error_E]

tau = cross (w , j*w) - jm * bw - psw + j*[-cw*ew];


% Individual motor inputs using control allocation
% equation-----------------------------

ac = [ftc tau1 tau2 tau3]
% enter manually ac values , or code it later!!!!
